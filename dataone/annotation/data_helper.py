...
class DataHelper():
    def __init__(self):
        self.manual_annotation_url = 'https://raw.githubusercontent.com/DataONEorg/semantic-query/master/lib/manual_annotation/joined_manual_annotations.csv'
    ...
    def get_manual_annotations(nSize=-1):
        resp = requests.get(self.manual_annotation_url)
    ...
    def get_label(self, manu_anno):
        try:
            result = ' '.join([manu_anno['text']['entityName'], manu_anno['text']['attributeName']])
            return result
        except KeyError:
            return ''

    def get_defn(self, manu_anno):
        try:
            result = manu_anno['text']['attributeDefinition']
            return result
        except KeyError:
            return ''

    def get_context(self, manu_anno):
        try:
            result = manu_anno['text']['entityDescription']
            return result
        except KeyError:
            return ''
