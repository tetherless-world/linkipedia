...
public class EntityLinkingWithValidationServletSurfaceSingle extends HttpServlet{
	...
	EntityLinker searcher = null ;
	 int validationSetNum = 5;
	  ...
	  public EntityLinkingWithValidationServletSurfaceSingle(String index){
		  searcher = new EntityLinker(index);
		  cache = new LRUCache(1000);

	  }
	  
	  public EntityLinkingWithValidationServletSurfaceSingle(String index, String sfIndex){
		  searcher = new EntityLinker(index);
		  cache = new LRUCache(1000);
			 try {
...
//				}
	  ...
	  public String getJsonResult(ArrayList<ArrayList<Annotation>> results, String query, int numReuslt, long time,AnnotationDecisionFilter filter){
			...
			for(ArrayList<Annotation> annotations:results){
				...
				annotation_result += "{";
				annotation_result += "\"entity_mention\": \""+annotations.get(0).getKeyword().replaceAll("\"", "") +"\",";
